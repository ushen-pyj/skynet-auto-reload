...
function sandbox.require(name)
	...
	if not filename then
		return error(string.format("module '%s' not found", name))
	end
	local print = reload.debug
	local file = io.open(filename, "r")
	if not file then
		return error(string.format("cannot open file '%s'", filename))
	end
	...
	local modfunc = load(filestr)
	if not modfunc then
		return error(string.format("load module '%s' failed", name))
	end
	local ok, mod = xpcall(modfunc, debug.traceback)
	if not ok then
		return error(mod)
	end
	local ret = mod
				...
				if type(new_one) == "table" and old_one then
					...
					for k,v in pairs(new_one) do
						if type(v) ~= "table" or	-- copy values not a table
							...
							if print then print("COPY", old_one, k) end
							old_one[k] = v
						end
					end
				end
